...
int N_samp = 1; // Number of sampling from the canonical distribution
double beta = 8;
...
int main()
{
    ...
    double** restrained_bead = new double*[N]; // centroid constrained bead position
    for (int i = 0 ; i < N ; ++i)
    {
        restrained_bead[i] = new double[P];
    }
    ...
    for (int rep = 0 ; rep < N_samp ; ++rep)
    {
        ...
        {
            cent_const_bead(restrained_bead[time], time, O_T, x_0, v_0, alpha);
            
            for (int k = 0 ; k < P ; ++k)
            {
                partial_force[time][k] = pot_deriv(restrained_bead[time][k]);
            }
            
            //print(restrained_bead[time]);
            //print(partial_force[time]);
            //cout << endl;
